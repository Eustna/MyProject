...
class getData(Dataset):
    def __init__(self,label_path,images_path):
        super().__init__()
        self.data = []
        path = convert_path(label_path)
        for i in os.listdir(path):
            self.data.append([convert_path(images_path)+"/"+i.split('.')[0]+'.png', path+'/'+i])
...
label_floder =os.path.join(root_folder, "train","label")
img_floder =os.path.join(root_folder, "train","train_pic")
origin_model_path =os.path.join(root_folder, "model",f"{model_name}.pth")
model_path = os.path.join(root_folder, "model",f"{model_name}.onnx")

data = getData(label_floder,img_floder)
if os.path.exists(origin_model_path):
    Train_model = torch.load(origin_model_path)
    print("检测到存在模型")
else:
    Train_model = mubModu()
    print("新建模型开始训练")
...
meLoss = mbLoss()
optm = torch.optim.Adam(Train_model.parameters(),lr=0.001)
maxLoss = 10000
...
for i in range(epoch):
    ...
    if sx/csl < maxLoss:
        torch.save(Train_model, origin_model_path)
        print("模型保存")
        maxLoss =sx/csl
#开始格式转换
...
try:
    Train_model = torch.load(origin_model_path, map_location=device)
    Train_model.to(device)
    Train_model.eval()
    ...
    torch.onnx.export(Train_model, x,model_path, input_names=input_names, output_names=output_names, verbose='True')
    print("成功把画框模型转onnx")
except:
    pass
